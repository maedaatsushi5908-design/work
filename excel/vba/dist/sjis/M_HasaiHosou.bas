...
'
' 「殻運搬（現場→処分地）」の枠（CELL_MAP を参照）も同じ直接参照。
'
'   I44 = As・車道・15㎝以下 → 舗装（集計）!M23
'   I45 = As・車道・15㎝超   → 舗装（集計）!M24
'   I46 = As・歩道・15㎝以下 → 舗装（集計）!M25
'   I48 = Co・舗装・15㎝以下 → 舗装（集計）!P23
'   I49 = Co・舗装・15㎝超   → 舗装（集計）!P24
'
...
'
' I44・I46・I48 は「殻運搬（現場→処分地）」の枠（As・車道／As・歩道／
' Co・舗装 の15㎝以下）で、舗装（集計）の M23（As車道<=15）・
' M25（As歩道、厚さでは分かれていない1本の値）・P23（Co舗装<=15）を
' 直接参照する。I45・I49 は同じ枠の「15㎝超」で、M24（As車道>15）・
' P24（Co舗装>15）を参照する。As・歩道の「15㎝超」（I47）は、
' 舗装（集計）側に対応するセル（M26）が空欄で厚さ別のデータが無いため
' 見送っている。Co・取壊（無筋/鉄筋、I50・I51）も 舗装（集計）に
